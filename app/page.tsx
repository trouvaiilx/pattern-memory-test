...
import { Check, X, RotateCcw, Download, AlertCircle, Activity } from 'lucide-react';
...
const PatternTester = () => {
  const [invalidPatterns, setInvalidPatterns] = useState(new Set());
  ...
  useEffect(() => {
    if (invalidPatterns.size > 0) {
      localStorage.setItem('invalidPatterns', JSON.stringify([...invalidPatterns]));
    }
  }, [invalidPatterns]);

  // Encode pattern as string
  const encodePattern = (pattern) => pattern.join('-');

  // Get middle dot between two dots (for auto-include rule)
  const getMiddleDot = (a, b) => {
  ...
  const addDotToPattern = (dotIndex) => {
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = validationResult === 'invalid' ? '#ef4444' : 
                      validationResult === 'valid' ? '#22c55e' :
                      validationResult === 'duplicate' ? '#f59e0b' : '#1f2937';
    ctx.lineWidth = 4;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    <div className="min-h-screen bg-white text-gray-900 p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-gray-100 border border-gray-300 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-gray-900">{stats.invalid}</div>
              ...
              <div
                ...
                className={`w-16 h-16 rounded-full transition-all duration-200 border-2 ${
                  currentPattern.includes(i)
                    ? validationResult === 'invalid' ? 'bg-red-500 border-red-600 scale-110' :
                      validationResult === 'valid' ? 'bg-green-500 border-green-600 scale-110' :
                      validationResult === 'duplicate' ? 'bg-yellow-500 border-yellow-600 scale-110' :
        ...
        {validationResult === 'duplicate' && (
          <div className="bg-yellow-50 border border-yellow-400 rounded-lg p-4 mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-700">You've marked this pattern as incorrect before</div>
            </div>
          </div>
        )}
        ...
        {validationResult === 'valid' && (
          <div className="bg-green-50 border border-green-500 rounded-lg p-4 mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-700">This is your correct pattern</div>
            </div>
          </div>
        )}
        ...
        {validationResult === 'invalid' && (
          <div className="bg-red-50 border border-red-500 rounded-lg p-4 mb-4 flex items-center gap-3">
            ...
            <div>
              <div className="font-semibold text-gray-900">Pattern Marked Invalid</div>
              <div className="text-sm text-gray-700">This pattern won't be suggested again</div>
            </div>
          </div>
        )}
        ...
        {awaitingValidation && (
          <div className="bg-gray-100 border border-gray-300 rounded-lg p-6 mb-4">
            <div className="text-center mb-4 font-semibold text-gray-900">Is this your correct pattern?</div>
            <div className="text-center mb-4 text-sm text-gray-600">Hold any dot to draw again</div>
        ...
        <div className="mt-6 text-center text-xs text-gray-500">
          🔒 All patterns are stored locally on your device only.<br />
